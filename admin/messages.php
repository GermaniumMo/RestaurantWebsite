...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'update_status') {
        $contactMessage->id = sanitize_input($_POST['id']);
        $contactMessage->status = sanitize_input($_POST['status']);
        
        if ($contactMessage->update_status()) {
...
$total_count = $contactMessage->getTotalCount();
$pagination = new Pagination($total_count, $items_per_page, $page);

// Get paginated messages
$stmt = $contactMessage->readAllPaginated($pagination->getLimit(), $pagination->getOffset());
...
<head>
    ...
    <title>Messages - Savoria Admin</title>
            ...
            <main class="col-md-10 ms-sm-auto px-md-4">
                <div class="py-4">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <button type="button" class="btn btn-sm btn-link p-0" 
                                                                onclick="showFullMessage('<?php echo htmlspecialchars($msg['message']); ?>')">
                                                            Read Full Message
                                                        </button>
                                                    </td>
                                                    <td><?php echo format_date($msg['created_at']); ?></td>
                                                    <td>
                                                        <span class="badge bg-<?php echo $msg['status'] === 'read' ? 'success' : ($msg['status'] === 'unread' ? 'warning' : 'info'); ?>">
                                                    ...
                                                    <td>
                                                        <div class="btn-group" role="group">
                                                            ...
                                                            <form method="POST" style="display: inline;">
                                                                ...
                                                                <button type="submit" class="btn btn-sm btn-info" title="Mark as Replied">
                    ...
                    <?php if ($pagination->getTotalPages() > 1): ?>
                        <div class="mt-4">
                            <?php echo $pagination->generatePaginationLinks('messages.php'); ?>
                        </div>
                    <?php endif; ?>
                </div>
            </main>
